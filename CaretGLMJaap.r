...
model_logGLM <- train(logprijs ~ Woonoppervlakte + factor(postcode4) + Type + Kamers + m2xm2 + Kamers*Woonoppervlakte + Garage + Period, data = bind_cols(X_train,Y_train), method='glm')
print(model_logGLM)

print("Simple multi-level model on Log(prijs)...")

model_MLM <- lmer(logprijs ~ (1|postcode4) + (1| Period) + (1|Type) + (1|Garage) + Woonoppervlakte + Kamers + m2xm2 + Kamers*Woonoppervlakte, data = bind_cols(X_train,Y_train))
print(model_MLM)
