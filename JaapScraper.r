...
# Broken down in two functions; one for getting the data from a single summary page and one for cleaning an manipulating an extracted data set
...
scrape_summary <- function(mainPage,p){
  ...
  paste("XXXXX Page: ",p," before fix XXXXX") %>% print()
  ...
  # Print lengths of feature vectors as a check
  paste("xxxxx Page: ",p," after fix xxxxx") %>% print()
  paste("Type: ",length(type_html)) %>% print()
  paste("Kamers: ",length(kamers_html)) %>% print()
  paste("Oppervlak: ",length(m2_html)) %>% print()
  paste("IDs: ",length(id_html)) %>% print()
...
clean_summary <- function(huizen_html){
  ...
  print(paste0("Rows with postal code: ",nrow(huizen_html)))
